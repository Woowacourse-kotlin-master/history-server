...
public class HeritageController {
    ...
    @Operation(summary = "문화재 이미지 인식 API", description = "업로드된 이미지로 문화재를 인식합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "문화재 이미지 인식 성공"),
            @ApiResponse(responseCode = "404", description = "유저 존재하지 않음")
    })
    @PostMapping(consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<HeritageImageRes> recognizeHeritageImage(
            @AuthenticationPrincipal String userId,
            @RequestPart MultipartFile heritageImage
    ) throws IOException {
